...
(function () {
  ...
  // ---------- Think cards: click to reveal (not hover) ----------
  // Each card is a real <button>, so Enter/Space already fire a click via
  // native behavior — no extra keydown handling needed. Clicking toggles
  // its own .is-open; clicking a different card closes whichever one was
  // open first; clicking outside the grid, or pressing Escape, closes it.
  var thinkCards = document.querySelectorAll(".think-card");
  if (thinkCards.length) {
    var closeAllThinkCards = function () {
      thinkCards.forEach(function (c) {
    ...
    document.addEventListener("click", function (e) {
      if (!e.target.closest(".think-card")) closeAllThinkCards();
    });
    document.addEventListener("keydown", function (e) {
      if (e.key === "Escape") closeAllThinkCards();
    });
  }
